...
- [x] **Approvals that reach the owner (Sep 2026)**: paused tool calls persist in `pending_confirmations`; jobs, watchers and system runs ask on the owner channel through the ledger instead of a dropped synthetic chat; yes/no or `/confirm <id>` from any owner chat; Approve / Deny in web chat and Settings > Approvals; per-tool flags, new rules (email, first contact, locks/covers/alarm/climate, appointments, deletes, `commitAndPush`) and a deny-list that holds in every mode. See `docs/security.md`, "Approvals".
