...
- **Dates**: `created_at` and `updated_at` are ISO UTC (`2026-09-22T17:06:14.000Z`). A new message moves its chat's `updated_at` forward (`touchSession`), never backwards, so the sidebar groups a chat by its last activity.
