...
- **Who owns a session**: `chat_sessions.source` says which interface the chat belongs to (`web`, `whatsapp`, `telegram`, `slack`, `scheduler`, `subagent`, `api`). The dashboard sidebar lists `source = 'web'` only. The column is set when the session is created; rows written before it existed are filled in at boot. An id that names its owner (`@`, `scheduled_`, `subagent-`, `api_`, all digits with or without a leading minus) decides first, then the commonest source among the chat's messages, and last the id shape: one with a dash reads as `web`, anything else as `unknown`. Only WhatsApp, Telegram, Slack, the scheduler and sub-agents keep chats of their own, so a source we do not know, such as a phone shortcut or the API gateway, leaves the chat in the owner's list as long as its id has the web shape. Before this column the list read the id shape, so a web chat that had been handed a Slack channel id (see below) dropped out of the sidebar. `SESSION_SOURCE_FILTER=0` goes back to the old id rule.
- **Empty chats are cleaned up** on navigation: a chat with no messages, no pin, the untouched title `New Chat` and no other interface behind it is deleted. A chat you named or cleared keeps its row, and a chat from WhatsApp, Telegram or Slack is left alone, because its row exists from the start of a turn and the first message lands later. `EMPTY_CHAT_CLEANUP=0` turns the cleanup off.
