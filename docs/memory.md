...
Every turn used to carry every fact in full: on the device, 642 rows, about 59,000 characters, roughly 14,800 tokens of a 45,000-token chat turn. `db.getFactsIndex()` now renders one line per fact inside one budget of 12,000 characters (`FACTS_INDEX_CHARS`, clamped to 2,000–40,000). The agent's notes take at most a quarter and only what they need; the owner's profile takes the rest. Measured on the device: 145 facts listed, about 12,000 characters, roughly 3,000 tokens, with 492 facts one call away.
...
- **A changed value drops a summary or a kind written for the old one**, so the block can never state a value that has been corrected.
- **Forgetting asks.** `forgetFact` carries a confirmation flag and counts as deleting the owner's data for the guardian, and a copy of the fact goes to `data/pruned_memories.json` first. `updateFact` and `forgetFact` match by key only, so a word in another fact's value can never rewrite or delete it, and a pinned fact needs `force`.
