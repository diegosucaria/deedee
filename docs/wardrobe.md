...
5.  **Virtual Mirror (1–4 Panels) — `visualize_outfit`**:
    -   **Photorealistic Render**: Uses the `IMAGE` role (default `gemini-3.1-flash-image`) with a stored reference selfie + garment crops.
...
-   **Models** (all via `configService.getModel(...)` — no hardcoded ids):
    ...
    -   **Virtual Mirror**: `IMAGE` (env `GEMINI_IMAGE_MODEL`, default `gemini-3.1-flash-image`). All roles: [models.md](models.md).
