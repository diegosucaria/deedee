...
1.  **Vinyl Ingestion ("Crate Digger")**:
    ...
    -   **Usage**: Send the photo in chat or WhatsApp and ask to add it, or upload it from the DJ Crate page. A photo on its own adds nothing: the crate is written only on the owner's ask, through the `add_vinyl` tool or the page. The tool reads the photos on the owner's message, or the latest photo he sent in the same chat within 30 minutes; only a chat turn in his own chat counts (`context.ownerTyped`), never a contact's, a group's, a job's or a watcher run's. The attachment analysis used to add every record it saw in a photo, ask or no ask, from any sender (a screenshot of a shop cart became eight "owned" records); `DJ_AUTO_INGEST=1` brings that back as it was. Read on every call, default off.
