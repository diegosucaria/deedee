...
1.  **Vinyl Ingestion ("Crate Digger")**:
    ...
    -   **Usage**: Send the photo in chat or WhatsApp and ask to add it, or upload it from the DJ Crate page. A photo on its own adds nothing: the crate is written only on the owner's ask, through the `add_vinyl` tool or the page. The tool reads the photos on the owner's message, or the latest photo he sent in the same chat within 30 minutes; only a chat turn in his own chat counts (`context.ownerTyped`), never a contact's, a group's, a job's, a watcher run's or a run resumed after an approval; the same holds for `image_path`. The attachment analysis used to add every record it saw in a photo, ask or no ask, from any sender (a screenshot of a shop cart became eight "owned" records); `DJ_AUTO_INGEST=1` brings that back as it was. Read on every call, default off.
...
| Tool | What it does |
|---|---|
| `add_vinyl` | Adds the records in the photos on the owner's current message (at most 5 photos per call), else the latest photo he sent in the chat within 30 minutes, or the image file at `image_path` (a file under `/app/data` only, resolved through `fs.realpath`, whose name and first bytes are an image's: a link named `x.jpg` to the credentials file is refused). Only on the owner's ask, and only from his own chat; a list he may buy is a search. Says which records were already in the crate. |
| `list_vinyls` | The crate, newest first. Each row says when it was added. |
| `search_vinyls` | One `query`, or a `queries` list with one entry per record, in one call; both count when both are given. Every word must match one of artist, title, label, catalog number or track names; punctuation-only words are ignored. When the words as typed match nothing, the search runs again without the number-only words (a price, a size, a year) and the reply marks it a near match. At most 50 searches per call and 10 hits shown per search; about 40,000 characters of hit lines per reply, shared evenly across the searches, and a query longer than 120 characters is cut where it is echoed. Each hit says when it was added. The tool-loop guard's same-tool warning starts at 20 calls for it (`apps/agent/src/utils/tool-loop-limits.js`; the default is 6); a run still ends at its own cap of 15 rounds. |
