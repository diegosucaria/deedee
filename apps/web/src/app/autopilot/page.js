...
function AutopilotPage() {
    ...
    const handleContactSelect = async (e) => {
        ...
        if (id === 'global') {
            ...
            // Find person to get relationship
            const person = settings.find(p => p.id === id);
            setSelectedRelationship(person?.relationship || '');
        }
    ...
    const handleSaveStyle = async () => {
        ...
        alert('Style & Relationship saved!');
                ...
                {activeTab === 'style' && (
                    <div className="space-y-6">
                        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                            ...
                            {selectedContactStyleId !== 'global' && (
                                <div className="mb-4 bg-zinc-950 p-4 rounded-lg border border-zinc-800/50">
                                    <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">
                                        Relationship Context (Guardrails)
                                    </label>
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            placeholder="e.g. Boss, Cousin, Wife, Gym Buddy..."
                                            className="flex-1 bg-zinc-900 border border-zinc-800 rounded px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:border-purple-500/50"
                                        />
                                        <div className="text-xs text-zinc-500 max-w-[200px] leading-tight flex items-center">
                                            Keywords like "Boss" or "Work" enforce formality. "Friend" allows slang.
                                        </div>
                                    </div>
                                </div>
                            )}
