...
import { RefreshCw, Loader2, AlertCircle, Trash2, CheckCircle2, Hash, Eye, Search, Lock, MessageCircle, Volume2, VolumeX, Plus, LogIn } from 'lucide-react';
...
// Slack tokens die every few weeks, so both the "add" and the "re-login" forms
// need the same steps and the same two fields. Keep one copy of each.
function TokenHelp({ compact = false }) {
    return (
        <ol className={`${compact ? 'text-[10px]' : 'text-xs'} text-zinc-400 space-y-1.5 list-decimal list-inside`}>
...
function TokenFields({ xoxc, xoxd, setXoxc, setXoxd, compact = false }) {
    const inputClass = compact
        ? 'w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white placeholder-zinc-600 focus:border-purple-500/50 focus:outline-none'
        : 'w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-600 focus:border-purple-500/50 focus:outline-none';
    const labelClass = compact
        ? 'text-[10px] font-medium text-zinc-400 block mb-1'
        : 'text-xs font-medium text-zinc-400 block mb-1';

    return (
        <div className={compact ? 'space-y-2' : 'space-y-3'}>
            <div>
                <label className={labelClass}>API Token (xoxc-)</label>
                <input
                    type="password"
                    value={xoxc}
            ...
            <div>
                <label className={labelClass}>Cookie Token (xoxd-)</label>
                <input
                    type="password"
                    value={xoxd}
...
function ConnectionCard({ conn, fetchStatus }) {
    ...
    const handleRelogin = async () => {
        ...
        // Pass teamId: the server keeps this workspace's monitored channels and
        // listening flag, and rejects tokens from another workspace.
        const res = await saveSlackCredentials(xoxc, xoxd, conn.teamId);
        if (res.success) {
            setXoxc('');
            setXoxd('');
            setReloginOpen(false);
            await fetchStatus();
            setMonitored(await getSlackMonitoredChannels(conn.teamId) || []);
        } else {
            setError(res.error || 'Re-login failed');
        }
        setRelogging(false);
    };
    ...
    return (
        <div className="bg-zinc-800/40 p-4 rounded-xl border border-zinc-700/50 space-y-4">
            ...
            {/* Expired tokens: the only fix is pasting fresh ones, so say that here. */}
            {!conn.connected && !reloginOpen && (
                <div className="bg-amber-500/10 border border-amber-500/20 text-amber-300 p-2.5 rounded-lg flex items-center justify-between gap-3">
                    <span className="flex items-center gap-2 text-xs">
                        <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
                        Slack signed this token out. Paste fresh tokens to read Slack again.
                    </span>
                    <button
                        onClick={() => setReloginOpen(true)}
                        className="text-[10px] px-2.5 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 rounded font-medium transition-colors flex-shrink-0"
                    >
                        Re-login
                    </button>
                </div>
            ...
            {reloginOpen && (
                <div className="p-3 bg-zinc-900/50 rounded-lg border border-zinc-700/50 space-y-3">
                    <div>
                        <h3 className="text-xs font-semibold text-white flex items-center gap-2">
                            <LogIn className="w-3.5 h-3.5 text-purple-400" />
                            Re-login to {conn.teamName}
                        </h3>
                        <p className="text-[10px] text-zinc-500 mt-1">
                            Monitored channels and the listening setting stay as they are.
                        </p>
                    </div>
                    <TokenHelp compact />
                    <TokenFields xoxc={xoxc} xoxd={xoxd} setXoxc={setXoxc} setXoxd={setXoxd} compact />
                    <div className="flex gap-2 pt-1">
                        <button
                            onClick={() => { setReloginOpen(false); setXoxc(''); setXoxd(''); setError(null); }}
                            disabled={relogging}
                            className="flex-1 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 rounded text-[10px] font-medium transition-colors disabled:opacity-50"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={handleRelogin}
                            disabled={relogging || !xoxc || !xoxd}
                        ...
                        >
                            {relogging ? <Loader2 className="w-3 h-3 animate-spin" /> : <LogIn className="w-3 h-3" />}
                            Reconnect
                        </button>
                    </div>
                </div>
            ...
            <div className="pt-3 border-t border-zinc-700/30 flex justify-between">
                <button
                    onClick={() => { setReloginOpen(true); setError(null); }}
                    disabled={busy || reloginOpen}
                    className="text-[10px] px-2 py-1 text-zinc-400 hover:text-purple-400 hover:bg-purple-500/10 rounded transition-colors flex items-center gap-1 disabled:opacity-40"
                >
        ...
        <div className="space-y-6">
            ...
            {(!hasConnections || showAdd) && (
                <div className="space-y-4 max-w-xl">
                    ...
                    <TokenFields xoxc={xoxc} xoxd={xoxd} setXoxc={setXoxc} setXoxd={setXoxd} />
