...
const proxyToAgent = async (req, res, method, path, data) => {
    try {
        const url = `${AGENT_URL}/internal/aliases${path}`;
        const response = await axios({ method, url, data, params: req.query });
        res.json(response.data);
    } catch (error) {
        console.error(`[API] Aliases Proxy Error (${method} ${path}):`, error.message);
...
router.delete('/:alias', (req, res) => proxyToAgent(req, res, 'DELETE', `/${req.params.alias}`, null));
