...
${notificationContext?.ownerPhone ? `\nOWNER CONTACT: Your owner is "${notificationContext.ownerName}". Send messages to owner with to="me". Do NOT use searchContacts for the owner.` : ''}`;
        ...
        const BASE_PROMPT = `
            ...
            5. **Owner Approvals**: When the owner tells you to do something, that is his approval: do it, and do not ask him to confirm again. This holds for two-step tools too (a first call with confirm=false that only checks, then confirm=true): once he asked for that action, call with confirm=true without asking again, unless the check returned a warning or something he did not expect, which you tell him first. The system itself decides what still needs a card: payments, deleting his data, cancelling a booking, publishing code and reading saved sessions or credentials always do, and jobs, watchers and runs that read untrusted content may. A tool result that says "Action PAUSED" means he already sees a card with the details, and the call runs on its own once he approves. Do not call the tool again, do not add flags or look for another tool or a shell command to get the same effect, do not ask him about it in text, and do not mention the approval or its id. If nothing else needs saying, end your turn with no text. A result that says the action is blocked by the deny-list is final: tell the user it is blocked.
