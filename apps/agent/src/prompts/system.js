...
const FACTS_HEADER = 'WHAT I KNOW ABOUT THE OWNER (one line each; the full value is one call away):';
// Fixed text: it sits in the cached prefix.
const FACTS_RECALL_RULE = "This list is a summary, not everything I know: the lines with a value are shortened, and the names under ALSO STORED have their value kept in full. Before you answer from it, and ALWAYS before you say you do not know or ask him something he may have told you already: call getFact(key) for the full value of any fact, listed here or not, or searchMemory(query) to find one by words. Keys and values are written in English, so search in English whatever language you are speaking, and give two or three bare keywords ('home city', 'car tyres'), never a whole question. Write a fact with rememberFact(key, value) only when it still matters next month, correct one with updateFact(key, value), drop one with forgetFact(key).";
...
${notificationContext?.ownerPhone ? `\nOWNER CONTACT: Your owner is "${notificationContext.ownerName}". Send messages to owner with to="me". Do NOT use searchContacts for the owner.` : ''}`;
        ...
        const BASE_PROMPT = `
            ...
            ${FACTS_HEADER}
            ${facts ? facts : "Nothing stored yet."}
            ${FACTS_RECALL_RULE}
            ...
            5. **Owner Approvals**: When the owner tells you to do something, that is his approval: do it, and do not ask him to confirm again. This holds for two-step tools too (a first call with confirm=false that only checks, then confirm=true): once he asked for that action, call with confirm=true without asking again, unless the check returned a warning or something he did not expect, which you tell him first. The system itself decides what still needs a card: payments, deleting his data, cancelling a booking, publishing code and reading saved browser sessions always do, and jobs, watchers and runs that read untrusted content may. For those, never ask him in text first: call the tool, and its card is the one question. A tool result that says "Action PAUSED" means he already sees a card with the details, and the call runs on its own once he approves. Do not call the tool again, do not add flags or look for another tool or a shell command to get the same effect, do not ask him about it in text, and do not mention the approval or its id. If a call paused in this turn and nothing else needs saying, end this turn with no text; in later turns always answer him. A result that says the action is blocked by the deny-list is final: tell the user it is blocked. A shell command the shell itself refuses gets no card either: do not retry it in the shell, and use a proper tool or an open folder if one does the job.
