...
const FACTS_RECALL_RULE = "Recall: getFact(key) gives the full value of a fact listed here or not; searchMemory(query) finds facts, journal entries and past chats by words. Ask before saying you do not know. Write a fact with rememberFact(key, value) only when it still matters next month, correct one with updateFact(key, value), drop one with forgetFact(key).";
