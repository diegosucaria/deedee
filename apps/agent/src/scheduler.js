...
1. Call get_user_profile. If morning_outfit_enabled is not truthy, output [SILENT] immediately and stop.
2. Look up the user's home city (searchMemory for "home city" / "lives in" / "location" — or getFact "home_city"). If unknown, proceed without weather.
3. If home city is known, spawn a FLASH sub-agent to fetch today's weather via the weather skill:
...
4. Spawn a FLASH sub-agent to summarize today's calendar for occasion/dress-code signal:
...
5. Call recommend_outfit(context: "today, <weather>, <occasion summary>", count: 2).
6. If get_user_profile.reference_image_path is set, optionally call visualize_outfit with the top proposal's garment_ids (single panel). If not set, skip the render.
7. Reply to the owner with the top proposal (1-2 sentences, cite specific pieces). Attach the rendered image if you produced one.
8. If recommend_outfit returned zero proposals, output [SILENT].
